...
function Calendar(element, instanceOptions) {
	...
	function renderView(delta, viewName) {
		...
		// if viewName is changing, destroy the old views
		...
		// if viewName changed, or the views was never created, create a fresh views
		if (!currentView && viewName) {
			currentView = new fcViews[viewName](t);
			currentView.el =  $("<div class='fc-views fc-" + viewName + "-views' />").appendTo(content);
	...
	// `viewName` can be a specific views name or a generic one like "week" or "day".
	function zoomTo(newDate, viewName) {
		var viewStr;
		var match;

		if (!viewName || fcViews[viewName] === undefined) { // a general views name, or "auto"
			...
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						else if (fcViews[buttonName]) { // a views name
							buttonClick = function() {
...
$.extend(Grid.prototype, {
	...
	// We don't need to undo any of this in a "destroy" method, because the views will simply remove `this.el` from the
	...
	segDragMousedown: function(seg, ev) {
		...
		var dragListener = new DragListener(view.coordMap, {
			...
			cellOver: function(cell, date) {
				...
				if (view.renderDrag(newStart, newEnd, seg)) { // have the views render a visual indication
					mouseFollower.hide(); // if the views is already using a mock event "helper", hide our own
				}
				else {
					mouseFollower.show();
				}
			...
			cellOut: function() { // called before mouse moves to a different cell OR moved out of all cells
				newStart = null;
				view.destroyDrag(); // unrender whatever was done in views.renderDrag
...
$.extend(DayGrid.prototype, {
	...
	numbersVisible: false, // should render a row for day/week numbers? manually set by the views
...
$.extend(TimeGrid.prototype, {
	...
	// Removes all event segment elements from the views
	destroyEvents: function() {
		Grid.prototype.destroyEvents.call(this); // call the super-method
...
View.prototype = {
	...
	intervalStart: null, // the start of the interval of time the views represents (1st of month for month views)
	intervalEnd: null, // the exclusive end of the interval of time the views represents
	...
	// Clears all views rendering, event elements, and unregisters handlers
	destroy: function() {
		this.unselect();
	...
	// Handler for accepting externally dragged events being dropped in the views.
...
function View(calendar) {
	...
	// Based off the settings of the views. Possible signatures:
...
$.extend(BasicView.prototype, {
	...
	// Renders a visual indication of an event being dragged over the views.
	// A returned value of `true` signals that a mock "helper" event has been rendered.
	renderDrag: function(start, end, seg) {
	...
	// Unrenders the visual indication of an event being dragged over the views
	destroyDrag: function() {
...
fcViews.month = MonthView; // register the views
...
fcViews.basicWeek = BasicWeekView; // register this views
...
fcViews.basicDay = BasicDayView; // register this views
...
$.extend(AgendaView.prototype, {
	...
	// Unrenders a visual indications of an event being dragged over the views
	destroyDrag: function() {
...
fcViews.agendaWeek = AgendaWeekView; // register the views
...
fcViews.agendaDay = AgendaDayView; // register the views
